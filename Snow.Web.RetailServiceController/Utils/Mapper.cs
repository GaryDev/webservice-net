...
namespace Snow.Web.RetailService
{
    ...
    public class CultureInfoTypeMapper : ITypeMapper<CultureInfo, CultureInfo>
    {
        public CultureInfo Map(CultureInfo source, CultureInfo target)
        {
            if (source == null) return null;

            return source.Clone() as CultureInfo;
        }
    }
        ...
        protected override bool Match(ConventionInfo c)
        {
            return
                ...
                (
                    ...
                    !c.SourceProp.Type.GetGenericArguments()[0].IsValueType &&
                    c.SourceProp.Type.GetGenericArguments()[0] != typeof(string) &&
                    !c.TargetProp.Type.GetGenericArguments()[0].IsValueType &&
                    c.TargetProp.Type.GetGenericArguments()[0] != typeof(string)
                 );
        }
    ...
    //e.g. AEnum -> BEnum
    public class EnumToEnum : ConventionInjection
    {
        protected override bool Match(ConventionInfo c)
        {
            return c.SourceProp.Name == c.TargetProp.Name &&
                c.SourceProp.Type.IsEnum && c.TargetProp.Type.IsEnum;
        }
